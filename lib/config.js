...
function getRaidenConfig() {
    ...
    try {
        raidenConfig = yaml.safeLoad(fs.readFileSync(`${process.env.HOME}/.raiden/config.yml`, 'utf8'));
    } catch (e) {
        return {};
    }
...
function getConfig() {
    ...
    const raidenConfig = _getRaidenConfig(),
          requestsFile = raidenConfig.reqfile || 'requests';
    ...
    try {
        ...
        const envs = sanitizeEnvs(
            yaml.safeLoad(fs.readFileSync(`${process.env.HOME}/.raiden/envs.yml`, 'utf8')),
            schemas.envsSanitizeSchema
        ),
        requests = sanitizeRequests(
            yaml.safeLoad(fs.readFileSync(`${process.env.HOME}/.raiden/${requestsFile}.yml`, 'utf8')),
