...
describe('lib/cmd/request', () => {
    ...
    it('RequestCommand skips invalid requests', (done) => {
        ...
        // make foo request invalid
        inspector.validate.mockReturnValueOnce({ valid: false, format: () => {} });
        inspector.validate.mockReturnValueOnce({ valid: true });

        rp.mockReturnValue(Promise.resolve());

        requestCommand = new RequestCommand(program).exec().then(() => {

            expect(rp.mock.calls[0][0]).toEqual(Object.assign({ jar: {} }, config.requests.bar));
