...
function createRequest(program, env, requestConfig) {
    ...
    request.headers = getHeaders(env, requestConfig, request.body);
    return request;
}
...
function getHeaders(env, r, body) {
   var base = body ? { 'Content-Length' : Buffer.byteLength(body) } : {};
   return Object.assign(base, r.headers, getCookies(env, r));
