...
function executeRequestsSync(program, queue) {
    return queue.reduce((sequence, request) => {
        return sequence.then(() => {
            ...
            return processResponse(program, { url: request.url, headers: request.config.headers, body: request.config.body }, resp);
        }).catch((e) =>{
            console.log(e);
        });
    }, Promise.resolve());
...
function executeRequestsAsync(program, queue) {
    ...
    queue.reduce((requests, request) => {
        var requestPromise = fetch(request.url, request.config)
            .then((resp) => processResponse(program, { url: request.url, headers: request.config.headers, body: request.config.body }, resp))
...
function processResponse(program, request, resp) {
    if (resp.headers.get('content-type') === 'application/json') {
        return resp.json().then((json) => {
            printer.print(program, request, resp, json); 
        });
    }
    return resp.text().then((text) => {
        printer.print(program, request, resp, text);
    });
}
