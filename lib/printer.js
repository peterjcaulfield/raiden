...
function printRequestInfo(program, request, resp, data) {
    ...
    if (program.headers) {
        _printHeaders(request.parsedConfig.headers, 'request');
        _printHeaders(resp.headers.raw(), 'response');
    } else if (program.body) {
        _printRequestBody(request);
        if (request.parsedConfig.method !== 'HEAD') {
...
function _printStatus(request, resp, request) {
    console.log(`${request.getExecutionTime()} [${resp.status}] ${request.parsedConfig.method} ${request.url}`);
}
...
function _printRequestBody(request) {
    ...
    if (request.parsedConfig.body instanceof FormData) {
      console.log(request.config.body);
    } else {
      console.log(request.parsedConfig.body);
    }
}
