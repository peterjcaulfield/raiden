...
class Request {
    ...
    _parseConfig(config, requestKey) {
        ...
        c.body = this._getBody(requestKey, config.headers, config.body, config);
    ...
    _getBody(requestKey, headers, body, config) {

        var contentType = helpers.getPropSafely(headers, 'content-type', '');
        ...
        if (contentType === 'application/json') {
            ...
            console.log(`[Error] \"${requestKey}\" request: A 'content-type' header is required if request body is an object:`);
