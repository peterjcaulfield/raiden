"use strict"

var fs = require('fs'),
    qs = require('qs'),
    helpers = require('./helpers'),
    chalk = require('chalk'),
    FormData = require('form-data'),
    transformers = require('./transformers');


class Request {

    constructor(requestKey, config, userArgs) {

        this.start;

        this.end;

        this.config = Object.assign({ env: '127.0.0.1' },
                                      config,
                                      helpers.cloneWithDefinedProps(userArgs));

        this.requestKey = requestKey;

        this.url =  this._getUrl();

        this.parsedConfig = this._parseConfig();
    }

    getExecutionTime() {
        return `${this.end[0]}s:${Math.round(this.end[1]/1000000)}ms`;
    }

    _parseConfig() {
        var c = {};
        c.method = this.config.method;
        c.body = this._getBody();
        c.headers = this._getHeaders(c.body);
        return c;
    }

    _getUrl() {
        return this.config.protocol + '//'
            + this.config.env + '/';
            + this.config.endpoint ? this.config.endpoint : ''
            + this.config.query ? '?' + this.config.query : '';
    }

    _applyTransforms() {

        if (this.config.transforms instanceof Array) {

            this.config.transforms.forEach((t) => {
                //TODO make this work with nested keys
                if (this.config.body.hasOwnProperty(t.key)) {
                    this.config.body[t.key] = transformers[t.transform[0]].apply(null, [t.transform[1]])
                } else {
                    console.log(`[Warn] Transform key \"${t.key}\" does not exist in \"${this.requestKey}\" request body. Skipping this transform.`);
                }

            });
        }

    }

    _getBody() {

        this._applyTransforms();

        var contentType = helpers.getPropSafely(this.config.headers, 'content-type', '');

        if (contentType === 'application/json') {

            return this._getJsonBody();

        } else if (contentType === 'application/x-www-form-urlencoded') {

            return this._getUrlEncodedBody();

        } else if (contentType === 'multipart/form-data') {

            return this._getMultipartBody();

        } else if (typeof this.config.body === 'object') {
            //TODO create error constants
            console.log(`[Error] \"${this.requestKey}\" request: A 'content-type' header is required if request body is an object:`);
            console.log(`${JSON.stringify(this.config, null, 4)}`);
            process.exit(1);
        }

        return this.config.body;
    }

    _getUrlEncodedBody() {
        return qs.stringify(this.config.body);
    }

    _getJsonBody() {
        return JSON.stringify(this.config.body);
    }

    _getMultipartBody() {

        return Object.keys(this.config.body).reduce((form, prop) => {

            var append = helpers.isFilePath(this.config.body[prop]) ?
                fs.createReadStream(helpers.getAbsoluteFilePath(this.config.body[prop])) :
                this.config.body[prop];

            form.append(prop, append);

            return form;

        }, new FormData());

    }

    _getHeaders(processedBody) {

        var contentHeaders;

        if (processedBody !== '') {
            if (processedBody instanceof FormData) {
                contentHeaders = processedBody.getHeaders();
            } else {
                contentHeaders = { 'Content-Length': Buffer.byteLength(processedBody) };
            }
        }

        return Object.assign(this.config.headers, contentHeaders || {});
    }
}

module.exports = Request;
