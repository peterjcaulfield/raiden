...
class Request {
    ...
    _parseConfig() {
        ...
        if (parsed.body) {
            ...
            parsed.formData = this._convertFilepathValuesToStreams(parsed.formData);
        }
    ...
    _convertFilepathValuesToStreams(obj) {

        return Object.keys(obj).reduce((form, prop) => {

            let data = helpers.isFilePath(obj[prop]) ?
                fs.createReadStream(helpers.getConfigFilePath() + obj[prop]) :
                obj[prop];
