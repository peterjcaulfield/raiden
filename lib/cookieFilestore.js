...
const tough = require('tough-cookie'),
      Store = tough.Store,
      permuteDomain = tough.premuteDomain,
...
class CookieFilestore extends Store {
    ...
    constructor() {
        ...
        this.filePath = `${helpers.getConfigFilePath()}cookies.json`;
    ...
    _cookieMap(func) {
        ...
        for (let domain in this.cookies) {
            ...
            for (let path in this.cookies[domain]) {
                ...
                for (let key in this.cookies[path]) {
        ...
        for (let domain in dataJson) {
            ...
            for (let path in dataJson[domain]) {
                ...
                for (let key in dataJson[domain][path]) {
                    ...
                    this.cookies[domain][path][key] = tough.fromJSON(JSON.stringify(dataJson[domain][path][key]));
