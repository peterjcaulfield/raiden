...
describe('lib/config', () => {
    ...
    it('it should call sanitize with empty objects if config files are empty', () => {

        const requestConfig = envConfig = raidenConfig = null;

        // return a raw config
        yaml.safeLoad.mockReturnValueOnce(raidenConfig);
        yaml.safeLoad.mockReturnValueOnce(envConfig);
        yaml.safeLoad.mockReturnValueOnce(requestConfig);

        // return a sanitized config
        inspector.sanitize.mockReturnValueOnce({ data: {} });
        inspector.sanitize.mockReturnValueOnce({ data: {} });
        inspector.sanitize.mockReturnValueOnce({ data: {} });
        ...
        expect(inspector.sanitize.mock.calls[0][1]).toEqual({});
        expect(config).toEqual({ envs: {}, requests: {}});
